...
```
# Câu trả lời: 
+ Do mở file bằng cờ O_APPEND nên khi dùng lseek ở bước 5, dù có cố gắng đưa về đầu file thì khi write dữ liệu vào vẫn sẽ được ghi vào cuối file.

# Hướng dẫn sử dụng code:

+ Chạy ```make``` để build code và ra file run chính là file thực thi.
+ ```make removeLogFile``` là để remove file log
+ ```make clean``` là để remove cả file log và file thực thi (file "run")
